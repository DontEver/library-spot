...
const FALLBACK_LIBRARIES = [
  { id: '18th-ave', name: '18th Avenue Library', address: '175 W. 18th Ave, Columbus, OH', rooms: [] },
  { id: 'thompson', name: 'Thompson Library', address: '1858 Neil Ave, Columbus, OH', rooms: [] },
  { id: 'faes', name: 'FAES Library', fullName: 'Food, Agricultural, and Environmental Sciences Library', address: '2120 Fyffe Rd, Columbus, OH', rooms: [] },
  { id: 'hsl', name: 'Health Sciences Library', subtitle: '(University Hospital)', address: '376 W. 10th Ave, Columbus, OH', bookingUrl: 'https://hsl-osu.libcal.com/spaces?lid=694&gid=24674', rooms: [] },
];
...
function roomHasEntireBlockFree(room, startMinutes, endMinutes, currentMinutes, isToday) {
  ...
  return slotsInRange.every(slot => {
    ...
    if (isToday) {
      const slotMinutes = parseTimeToMinutes(slot.time);
      if (slotMinutes !== null && slotMinutes + 30 <= currentMinutes) return false;
    }
    return true;
  });
}
...
function roomHasConsecutiveFree(room, requiredMinutes, currentMinutes, isToday) {
  if (!room.slots || room.slots.length === 0) return false;
...
function getConsecutiveFreeBlocks(room, requiredMinutes, currentMinutes, isToday) {
...
function HslHoursDisplay({ selectedDate }) {
  ...
  return (
    <div className="bg-slate-700/30 rounded-lg p-3 mb-4">
      <p className="text-xs text-slate-400 mb-2 font-medium">HSL Hours This Week</p>
      <div className="grid grid-cols-7 gap-1">
        {days.map((day) => (
          ...
          >
            <div className={`text-xs font-medium ${day.isSelected ? 'text-indigo-300' : 'text-slate-400'}`}>
              {day.dayName}
            </div>
            <div className={`text-[10px] mt-1 ${day.isSelected ? 'text-slate-200' : 'text-slate-500'}`}>
              {day.hours.open}
            </div>
            <div className={`text-[10px] ${day.isSelected ? 'text-slate-200' : 'text-slate-500'}`}>
              {day.hours.close}
            </div>
            {day.hours.note && (
              <div className="text-[9px] text-amber-400 mt-0.5 truncate" title={day.hours.note}>
                {day.hours.note}
              </div>
            )}
          </div>
...
function TimeFilter({ startTime, endTime, onStartChange, onEndChange, onClear }) {
  // All end times after start (no max limit)
  const validEndOptions = TIME_OPTIONS.filter(opt => 
    startTime === null || opt.value > startTime
  );
...
function DurationFilter({ duration, onDurationChange, onClear }) {
  return (
    <div className="flex flex-wrap items-center gap-3">
      <div className="flex items-center gap-2">
        <label className="text-sm text-slate-400">Minimum consecutive free:</label>
        <select
          value={duration ?? ''}
...
function TimeSlotGrid({ slots, currentTime, isToday, isClosed, timeFilter, durationFilter, room }) {
  if (isClosed) {
    return <p className="text-sm text-slate-500 italic">Closed</p>;
  }
  
  if (!slots || slots.length === 0) {
    return <p className="text-sm text-slate-500 italic">No reservations available</p>;
  }
  ...
  if (durationFilter !== null && room) {
    const blocks = getConsecutiveFreeBlocks(room, durationFilter, currentMinutes, isToday);
    
    if (blocks.length === 0) {
      return <p className="text-sm text-slate-500 italic">No consecutive {durationFilter / 60}h+ blocks available</p>;
    }
    ...
    return (
      <div className="space-y-2">
        {blocks.map((block, blockIdx) => {
          ...
          const endHourDisplay = endHour > 12 ? endHour - 12 : endHour === 0 ? 12 : endHour;
          const endTime = `${endHourDisplay}:${endMin.toString().padStart(2, '0')}${endPeriod}`;
          const duration = block.length * 30;
          const durationStr = duration >= 60 ? `${duration / 60}h` : `${duration}min`;
          
          return (
            <div key={blockIdx} className="bg-emerald-600/20 border border-emerald-500/30 rounded-lg p-2">
  ...
  if (filteredSlots.length === 0) {
    if (timeFilter.start !== null || timeFilter.end !== null) {
      return <p className="text-sm text-slate-500 italic">No slots in selected time range</p>;
    }
    return <p className="text-sm text-slate-500 italic">{isToday ? 'Closed for today' : 'No reservations available'}</p>;
  }
  ...
  const isInFilterRange = (slotTime) => {
    ...
    return slotMinutes !== null && slotMinutes >= timeFilter.start && slotMinutes < timeFilter.end;
  };
...
function RoomCard({ room, currentTime, isToday, isClosed, timeFilter, durationFilter }) {
  ...
  const relevantSlots = room.slots?.filter(slot => {
  ...
  const availableCount = isClosed ? 0 : relevantSlots.filter(s => s.available).length;
  const nextAvailable = isClosed ? null : relevantSlots.find(s => s.available);
  
  // Get consecutive blocks if duration filter is active
  const consecutiveBlocks = durationFilter !== null 
    ...
    <div className="bg-slate-800 rounded-lg p-4 border border-slate-700">
      <div className="flex justify-between items-start mb-3">
        <div>
          ...
          <p className="text-sm text-slate-400">Floor {room.floor} • {room.capacity} people</p>
        </div>
        <div className="text-right">
          {isClosed ? (
            ...
            <>
              <span className={`text-sm font-medium ${consecutiveBlocks.length > 0 ? 'text-emerald-400' : 'text-red-400'}`}>
                {consecutiveBlocks.length > 0 ? `${consecutiveBlocks.length} block${consecutiveBlocks.length > 1 ? 's' : ''}` : 'No blocks'}
              </span>
              <p className="text-xs text-slate-500">{durationFilter / 60}h+ consecutive</p>
            </>
          ) : (
            <>
				<span className={`text-sm font-medium ${availableCount > 0 ? 'text-emerald-400' : 'text-red-400'}`}>
				  {availableCount > 0 ? `${availableCount} ${availableCount === 1 ? 'slot' : 'slots'}` : 'Full'}
				</span>
              {nextAvailable && (
                <p className="text-xs text-slate-500">Next: {formatTimeDisplay(nextAvailable.time)}</p>
              )}
            </>
          )}
        </div>
      ...
      <div className="flex items-center gap-2 mb-3 text-sm text-slate-400">
        {room.amenities?.map(a => (
          <span key={a} title={a}>{AMENITY_ICONS[a] || a}</span>
        ))}
      </div>
      
      <TimeSlotGrid slots={room.slots} currentTime={currentTime} isToday={isToday} isClosed={isClosed} timeFilter={timeFilter} durationFilter={durationFilter} room={room} />
    </div>
  );
}

function LibrarySection({ library, currentTime, expanded, onToggle, isToday, timeFilter, durationFilter, selectedDate }) {
  ...
  if (library.hours && !isHSL) {
    ...
    if (reservationHours) {
      ...
      } else if (reservationHours.open !== null && reservationHours.close !== null) {
        hours = {
          ...hours,
  ...
  const isPastClosing = isToday && (isClosedForDay || currentHour >= hours.close);
  const isBeforeOpening = isToday && !isClosedForDay && currentHour < hours.open;

  // Filter rooms based on filter criteria
  const filteredRooms = library.rooms?.filter(room => {
    // If time range filter is set, check if room has entire block free
    if (timeFilter.start !== null && timeFilter.end !== null) {
      if (!roomHasEntireBlockFree(room, timeFilter.start, timeFilter.end, currentMinutes, isToday)) {
        return false;
      }
    }
    ...
    if (durationFilter !== null) {
      if (!roomHasConsecutiveFree(room, durationFilter, currentMinutes, isToday)) {
        return false;
      }
    }
  ...
  const totalAvailable = isPastClosing ? 0 : filteredRooms.reduce((acc, room) => {
    const slots = room.slots?.filter(s => {
  ...
  const firstAvailableSlot = (() => {
    ...
    filteredRooms.forEach(room => {
      room.slots?.forEach(slot => {
        ...
        // Apply time filter
        if (timeFilter.start !== null && parsed.total < timeFilter.start) return;
  ...
  const getBadgeInfo = () => {
    ...
    if (isToday) {
      if (isPastClosing) return { text: 'Closed', color: 'bg-slate-600/20 text-slate-400' };
      if (isBeforeOpening) return { text: 'Opens Soon', color: 'bg-amber-600/20 text-amber-400' };
    ...
    <div className="mb-4">
      ...
      >
          ...
          <div className="text-left">
            <div className="flex items-center gap-2">
              <h2 className="font-semibold text-white">
                {library.name}
                {library.subtitle && <span className="text-slate-400 font-normal ml-1">{library.subtitle}</span>}
              </h2>
              <span className={`px-2 py-0.5 text-xs rounded ${badge.color}`}>{badge.text}</span>
              {library.isLive && badge.text === 'Open' && (
                <span className="px-2 py-0.5 bg-emerald-600/20 text-emerald-400 text-xs rounded">Live</span>
              )}
            </div>
            {library.fullName && <p className="text-xs text-slate-500">{library.fullName}</p>}
        ...
        <div className="flex items-center gap-4">
            ...
            {(isPastClosing || isClosedForDay) ? (
              <>
                ...
                {isToday ? (
                  ...
                  <p className="text-sm text-slate-500">{filteredRooms.length} rooms</p>
                )}
              </>
            ) : (
              <>
                <span className={`font-medium ${totalAvailable > 0 ? 'text-emerald-400' : 'text-red-400'}`}>{totalAvailable} slots</span>
                {firstAvailableSlot ? (
                  <p className="text-sm text-slate-400">First available: {formatTimeDisplay(firstAvailableSlot.time)}</p>
                ) : (
                  <p className="text-sm text-slate-500">{filteredRooms.length} rooms</p>
                )}
              </>
          ...
          <span className={`transition-transform text-slate-400 ${expanded ? 'rotate-180' : ''}`}>
            ▼
          </span>
        </div>
      </button>
        ...
        <div className="mt-4 pl-4">
          ...
          {!isHSL && (
            <div className="text-xs text-slate-500 mb-3">
              {buildingHoursStr && (
                <p>Building Hours: {buildingHoursStr}</p>
              )}
              {hours.name && (
                <p>Reservation Hours: {hours.name}</p>
              )}
            </div>
          )}
          ...
          {library.rooms?.length === 0 ? (
            <p className="text-slate-500 italic mb-4">{(isPastClosing || isClosedForDay) ? 'Closed for today' : 'No room data available'}</p>
          ) : filteredRooms.length === 0 && (timeFilter.start !== null || durationFilter !== null) ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {filteredRooms.map((room, idx) => (
                <RoomCard key={idx} room={room} currentTime={currentTime} isToday={isToday} isClosed={isPastClosing || isClosedForDay} timeFilter={timeFilter} durationFilter={durationFilter} />
              ))}
            </div>
          )}
...
export default function App() {
  ...
  const [currentTime, setCurrentTime] = useState({ hour: 12, minute: 0, display: 'Loading...' });
  ...
  const filteredLibraries = libraries.filter(lib => libraryFilter.includes(lib.id));
  ...
  useEffect(() => {
    ...
    const displayInterval = setInterval(updateDisplay, 200);
  ...
  // Fetch library data when date changes
  // Cache all 8 days and refresh every minute
  const [libraryCache, setLibraryCache] = useState({});
  const [isRefreshing, setIsRefreshing] = useState(false);
  
  // Function to fetch all days
  const fetchAllDays = async (isBackground = false) => {
    if (!isBackground) {
      setLoading(true);
    } else {
    ...
    try {
      ...
      const fetchPromises = days.map(day => 
        fetch(apiUrl(`/api/libraries?date=${encodeURIComponent(day.dateStr)}`))
      ...
      if (hasData) {
        setLibraryCache(prev => ({ ...prev, ...newCache }));
        setApiStatus('online');
      } else if (!isBackground) {
        // Only use fallback on initial load failure
        const fallbackCache = {};
        days.forEach(day => {
          fallbackCache[day.dateStr] = {
            data: FALLBACK_LIBRARIES.map(lib => ({ ...lib, isDemo: true })),
            fetchedAt: Date.now(),
          };
        });
        setLibraryCache(fallbackCache);
  ...
  // Initial fetch of all days
  useEffect(() => {
    fetchAllDays(false);
    
    // Refresh all days every minute
  ...
  const stats = useMemo(() => {
    ...
    filteredLibraries.forEach(lib => {
      ...
      if (lib.hours && lib.id !== 'hsl') {
        ...
        if (reservationHours) {
          if (reservationHours.closed) {
            ...
            hours = { ...hours, open: reservationHours.open, close: reservationHours.close };
          }
        }
      }
      ...
      // Filter rooms based on filter criteria
      const filteredRooms = lib.rooms?.filter(room => {
        if (timeFilter.start !== null && timeFilter.end !== null) {
          if (!roomHasEntireBlockFree(room, timeFilter.start, timeFilter.end, currentMinutes, isToday)) {
            return false;
          }
      ...
      availableSlots += filteredRooms.reduce((a, r) => {
        const slots = r.slots?.filter(s => {
          ...
          if (timeFilter.start !== null && parsed.total < timeFilter.start) return false;
          if (timeFilter.end !== null && parsed.total >= timeFilter.end) return false;
  ...
  const selectedDayLabel = days.find(d => d.dateStr === selectedDate)?.fullLabel || '';
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white">
      ...
      <header className="bg-slate-900 border-b border-slate-800 sticky top-0 z-40">
        <div className="max-w-6xl mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              <div className="text-right">
                <p className="text-sm text-white font-medium">{currentTime.display}</p>
                <p className="text-xs text-slate-500">
                  {isToday ? 'Viewing Today' : `Viewing ${selectedDayLabel}`}
                  {isRefreshing && <span className="ml-2 text-emerald-400">↻</span>}
                </p>
              </div>
              <div className={`w-2 h-2 rounded-full ${isRefreshing ? 'animate-spin' : 'animate-pulse'} ${
                apiStatus === 'online' ? 'bg-emerald-500' : 
                apiStatus === 'offline' ? 'bg-amber-500' : 'bg-slate-500'
              }`} />
            </div>
          </div>
        </div>
      ...
      <main className="max-w-6xl mx-auto px-4 py-6">
        ...
        <div className="mb-4">
          <p className="text-sm text-slate-400 mb-2">Find Rooms Free for Entire Time Block</p>
          <TimeFilter
            startTime={timeFilter.start}
            endTime={timeFilter.end}
            onStartChange={(start) => setTimeFilter(prev => ({ 
              start, 
              end: start === null ? null : (prev.end && prev.end <= start ? null : prev.end)
            }))}
        ...
        <div className="mb-4">
          <p className="text-sm text-slate-400 mb-2">Or Find Rooms with Consecutive Free Time</p>
          <DurationFilter
        ...
        {apiStatus === 'offline' && (
          <div className="bg-amber-600/10 border border-amber-600/30 rounded-lg p-4 mb-6">
            <div className="flex items-start gap-3">
              ...
              <div>
                <p className="text-amber-400 font-medium">Backend not running</p>
                <p className="text-sm text-amber-400/70 mt-1">
                  Start the server with: <code className="bg-slate-800 px-2 py-0.5 rounded">cd server && node index.js</code>
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="grid grid-cols-3 gap-4 mb-6">
          <div className="bg-slate-800 rounded-lg p-4 text-center">
            <p className="text-2xl font-bold text-white">{filteredLibraries.length}</p>
          ...
          <div className="bg-slate-800 rounded-lg p-4 text-center">
            <p className={`text-2xl font-bold ${stats.availableSlots > 0 ? 'text-emerald-400' : 'text-red-400'}`}>{stats.availableSlots}</p>
        ...
        {loading && (
          <div className="text-center py-12">
            ...
            <p className="text-slate-400">Loading availability for {selectedDayLabel}...</p>
          </div>
        )}
